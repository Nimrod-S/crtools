...
cumspec_pro2 = np.load("/tmp/protonsyo.npy")
...
for md in range(1):
    ...
    cumspec_nuc = np.load(f"../nuc{nm}yo.npy")
    cumspec_cno = np.load(f"../cno{nm}yo.npy")
    cumspec_sil = np.load(f"../sil{nm}yo.npy")
    cumspec_irn = np.load(f"../irn{nm}yo.npy")

    spec_nuc = -np.gradient(cumspec_nuc, e0s)
    spec_cno = -np.gradient(cumspec_cno, e0s)
    spec_sil = -np.gradient(cumspec_sil, e0s)
    spec_irn = -np.gradient(cumspec_irn, e0s)

    plt.plot(np.log10(e0s), np.log10(spec_nuc * e0s**3), color='black', linestyle=lnstyle, label="nuclei model (total)", linewidth=3, alpha=1)

    plt.plot(np.log10(e0s), np.log10(spec_irn * e0s**3), color='purple', linestyle=lnstyle, label="$A_o>38$", linewidth=3)
    plt.plot(np.log10(e0s), np.log10(spec_sil * e0s**3), color='blue', linestyle=lnstyle, label="$A_o=24-38$", linewidth=3)
    plt.plot(np.log10(e0s), np.log10(spec_cno * e0s**3), color='green', linestyle=lnstyle, label="$A_o=5-23$", linewidth=3)
